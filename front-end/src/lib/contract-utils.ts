...
export const executeContractCall = async (
  ...
  const contractCallTxOptions = {
    ...
    postConditions: txOptions.postConditions as PostCondition[] | undefined,
    postConditionMode: PostConditionMode.Allow,
    fee: 1000,
  } as unknown as SignedContractCallOptions;

  const transaction = await makeContractCall(contractCallTxOptions);

  const response = await broadcastTransaction({ transaction, network: DEVNET_NETWORK });

  if ("error" in response) {
    // Log structured error and throw
    console.error("broadcastTransaction error:", response.error);
