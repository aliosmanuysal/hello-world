  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const doSuccessToast = (txid: string) => {
        toast({
          title: "Thank you for supporting Technology for Good!",
          description: (
            <Flex direction="column" gap="3">
              <Box>We received your contribution of ${amount}.</Box>
              <Box>
                Your support helps bring connectivity to rural classrooms and
                funds digital literacy courses for students.
              </Box>
              <Box fontSize="xs">
                Transaction ID: <strong>{txid}</strong>
              </Box>
              <Box fontSize="sm" color="gray.700">
                Success story preview: &quot;A connected classroom in Village X now
                runs weekly digital workshops — 12 students completed their
                first module this month.&quot; — updates will appear on the campaign
                page as milestones are reached.
              </Box>
            </Flex>
          ),
          status: "success",
      ...
      // Show user-facing error and log to console for debugging
      toast({
        title: "Error",
        description: (e as Error)?.message || "Failed to make contribution",
        status: "error",
        isClosable: true,
      });
